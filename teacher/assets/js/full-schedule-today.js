(() => {
  ...
  const applyTodayMarkerForFullSchedule = (table) => {
    if (!table || !table.classList.contains("class-schedule-table")) {
      return;
    }
    ...
    if (!hasParseableDate || todayMatchCount > 0) {
      return;
    }
  ...
  const applyTodayMarkersForFullSchedule = () => {
    ...
    document
      ...
      .forEach((table) => applyTodayMarkerForFullSchedule(table));
  };
