...
\begin{document}
	...
	\section*{Solutions: Characterization}
	...
	\begin{center}
		\begin{tabularx}{\textwidth}{|C{1.2cm}|C{2.3cm}|C{2.5cm}|>{\raggedright\arraybackslash}X|}
			...
			Discrete &
			Bernoulli &
			There is one situation with exactly two possible outcomes: the payment executes or remains unexecuted. \\
			\hline
			
			2 &
			Continuous &
			Uniform &
			The variable is bounded between negative 1.5 percent and positive 1.5 percent, with no part of the range expected more often than another. \\
			\hline
			
			3 &
			Discrete &
			Binomial &
			The variable counts how many purchases occur across a fixed number of 60 comparable travellers. \\
			\hline
			
			4 &
			Discrete &
			Poisson &
			The variable counts how many separate breakdown reports occur during a fixed period and exposure. \\
			\hline
			
			5 &
			Continuous &
			Triangular &
			The variable has a practical minimum, a practical maximum, and one most common value between them. \\
			\hline
			
			6 &
			Discrete &
			Geometric &
			The variable counts how many applications are required until the first approval occurs. \\
			\hline
			
			7 &
			Continuous &
			Linear &
			The variable is bounded and becomes steadily more common as its value increases across the range. \\
			\hline
			
			8 &
			Discrete &
			Binomial &
			The variable counts how many purchases occur across a fixed number of 100 comparable customers. \\
			\hline
			
			9 &
			Continuous &
			Normal &
			Values gather around a typical level and become increasingly uncommon farther from that level on either side. \\
			\hline
			
			10 &
			Discrete &
			Poisson &
			The variable counts how many separate cancellations occur during a fixed two-hour period. \\
			\hline
