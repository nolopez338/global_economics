...
\begin{document}
	\ExamTitleBlock{10th grade}{Term 1 C6 Catch-Up Activity (Solutions)}{}
	...
	The minimax regret choice is Package D because it has the smallest maximum regret (24).
		...
		\[
		\begin{aligned}
			\text{Rapid expansion: } &\max\{260, 248, 242, 234, 226\} = 260, \\
			\text{Steady growth: } &\max\{220, 222, 218, 212, 206\} = 222, \\
			\text{Flat market: } &\max\{170, 182, 188, 190, 188\} = 190, \\
			\text{Mild contraction: } &\max\{118, 132, 146, 156, 162\} = 162, \\
			\text{Strong contraction: } &\max\{70, 86, 102, 118, 130\} = 130, \\
			\text{Recovery transition: } &\max\{136, 148, 158, 164, 168\} = 168. \\
		\end{aligned}
		\]
		...
		\begin{tabular}{l c c c c c c c}
			...
			& \shortstack{Maximum\\regret} \\
			\midrule
			Design A & $260-260=0$ & $222-220=2$ & $190-170=20$ & $162-118=44$ & $130-70=60$ & $168-136=32$ & 60 \\
			Design B & $260-248=12$ & $222-222=0$ & $190-182=8$ & $162-132=30$ & $130-86=44$ & $168-148=20$ & 44 \\
			Design C & $260-242=18$ & $222-218=4$ & $190-188=2$ & $162-146=16$ & $130-102=28$ & $168-158=10$ & 28 \\
			Design D & $260-234=26$ & $222-212=10$ & $190-190=0$ & $162-156=6$ & $130-118=12$ & $168-164=4$ & 26 \\
			Design E & $260-226=34$ & $222-206=16$ & $190-188=2$ & $162-162=0$ & $130-130=0$ & $168-168=0$ & 34 \\
			\bottomrule
		\end{tabular}
...
\section*{Problem 1}
\[
R_{\max}(\text{Rent kiosk})=0
\]
\[
\min R_{\max}=0\;\Rightarrow\;\text{Minimax Regret choice: Rent kiosk}
\]

\section*{Problem 2}
\[
R_{\max}(\text{Plan A})=12,\quad R_{\max}(\text{Plan B})=12
\]
\[
\min R_{\max}=12\;\Rightarrow\;\text{Minimax Regret choice: Plan A and Plan B}
\]

\section*{Problem 3}
\[
R_{\max}(\text{Mode A})=12,\quad R_{\max}(\text{Mode B})=7
\]
\[
\min R_{\max}=7\;\Rightarrow\;\text{Minimax Regret choice: Mode B}
\]

\section*{Problem 4}
\[
R_{\max}(\text{Strategy A})=26,\quad R_{\max}(\text{Strategy B})=12,\quad R_{\max}(\text{Strategy C})=24
\]
\[
\min R_{\max}=12\;\Rightarrow\;\text{Minimax Regret choice: Strategy B}
\]

\section*{Problem 5}
\[
R_{\max}(\text{Policy A})=26,\quad R_{\max}(\text{Policy B})=10,\quad R_{\max}(\text{Policy C})=0
\]
\[
\min R_{\max}=0\;\Rightarrow\;\text{Minimax Regret choice: Policy C}
\]

\section*{Problem 6}
\[
R_{\max}(\text{Format A})=36,\quad R_{\max}(\text{Format B})=18,\quad R_{\max}(\text{Format C})=6,\quad R_{\max}(\text{Format D})=30
\]
\[
\min R_{\max}=6\;\Rightarrow\;\text{Minimax Regret choice: Format C}
\]

\section*{Problem 7}
\[
R_{\max}(\text{Contract A})=46,\quad R_{\max}(\text{Contract B})=30,\quad R_{\max}(\text{Contract C})=16,\quad R_{\max}(\text{Contract D})=28
\]
\[
\min R_{\max}=16\;\Rightarrow\;\text{Minimax Regret choice: Contract C}
\]

\section*{Problem 8}
\[
R_{\max}(\text{Plan A})=64,\quad R_{\max}(\text{Plan B})=42,\quad R_{\max}(\text{Plan C})=24,\quad R_{\max}(\text{Plan D})=22,\quad R_{\max}(\text{Plan E})=34
\]
\[
\min R_{\max}=22\;\Rightarrow\;\text{Minimax Regret choice: Plan D}
\]

\section*{Problem 9}
\[
R_{\max}(\text{Package A})=60,\quad R_{\max}(\text{Package B})=42,\quad R_{\max}(\text{Package C})=28,\quad R_{\max}(\text{Package D})=24,\quad R_{\max}(\text{Package E})=34
\]
\[
\min R_{\max}=24\;\Rightarrow\;\text{Minimax Regret choice: Package D}
\]

\section*{Problem 10}
\[
R_{\max}(\text{Design A})=60,\quad R_{\max}(\text{Design B})=44,\quad R_{\max}(\text{Design C})=28,\quad R_{\max}(\text{Design D})=26,\quad R_{\max}(\text{Design E})=34
\]
\[
\min R_{\max}=26\;\Rightarrow\;\text{Minimax Regret choice: Design D}
\]
