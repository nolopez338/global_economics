...
The argument \(A_i\) identifies an alternative-based component, the argument
\(S_j\) identifies a state-based component, and the argument
\((A_i,S_j)\) identifies a component that may depend on both the alternative
and the state. Thus, \(q(A_i,S_j)\), \(r(A_i,S_j)\), and \(c(A_i,S_j)\) may
change in every cell. A fixed lump sum is not multiplied by quantity. A
component absent from one model is zero. A directly supplied
\(R(A_i,S_j)\) or \(C(A_i,S_j)\) needs no decomposition.
...
\begin{enumerate}[label=\arabic*.]
	...
	\item Classify each item as variable or fixed; identify whether each
	variable rate or fixed component is cell-specific, alternative-based, or
	state-based.
...
\textbf{Paired ladder overview.} AV, SV, CV, AF, and SF denote
alternative-variable, state-variable, cell-variable, alternative-fixed, and
state-fixed components. Within each tier, the cost list is a deliberate
permutation of the revenue list; matching structures are not assumed.
...
\begin{center}
	...
	\begin{tabularx}{\textwidth}{@{}lXX@{}}
		...
		Two components
		& AV+SV; AV+CV; AV+AF; AV+SF; SV+CV; SV+AF; SV+SF; CV+AF; CV+SF; AF+SF
		& AV+SF; SV+AF; CV+SF; SV+CV; AF+SF; AV+SV; AV+CV; AV+AF; SV+SF; CV+AF \\
		
		Three components
		& AV+SV+CV; AV+SV+AF; AV+SV+SF; AV+CV+AF; AV+CV+SF;
		AV+AF+SF; SV+CV+AF; SV+CV+SF; SV+AF+SF; CV+AF+SF
		& AV+SV+SF; AV+CV+AF; SV+CV+SF; AV+AF+SF; SV+AF+SF;
		CV+AF+SF; AV+SV+CV; AV+SV+AF; AV+CV+SF; SV+CV+AF \\
...
In thousand USD, the revenue rates for each alternative--state pair are
\[
\begin{aligned}
	r(A_1,S_1)&=15,
	&
...
The cost rates for each alternative--state pair are
\[
\begin{aligned}
	c(A_1,S_1)&=4,
	&
...
In the complete model, the quantities determining revenue and cost may depend
jointly on both the production alternative and the market state. Specifically,
the model includes:

\begin{enumerate}[label=\arabic*.]
	\item the number of units sold, \(q(A_i,S_j)\);
	\item the revenue per unit, \(r(A_i,S_j)\);
	\item the variable cost per unit, \(c(A_i,S_j)\);
	\item the fixed revenue, \(f_r(A_i,S_j)\);
	\item the fixed cost, \(f_c(A_i,S_j)\).
\end{enumerate}
...
Thus, every component of the revenue and cost calculation is specified
directly for each alternative--state pair. This allows the model to represent
situations in which production quantities, unit revenues, unit costs, fixed
revenues, and fixed costs are jointly affected by the selected alternative and
the realized state.

The three problems in this section use \(2\times2\), \(3\times2\), and
\(2\times3\) alternative--state structures, respectively. All monetary
quantities are expressed in million COP.
...
and input-market conditions are known. The company wants to estimate the profit
associated with each possible combination of production system and market
conditions. All monetary amounts in the problem are expressed in million
Colombian pesos (million COP).

The company is considering two production alternatives:

\begin{itemize}
	\item $A_1$: conventional assembly system.
	\item $A_2$: automated assembly system.
\end{itemize}

The market can develop under two possible states:

\begin{itemize}
	\item $S_1$: strong demand and favourable input-market conditions.
	\item $S_2$: limited demand and expensive input-market conditions.
\end{itemize}

For each combination of production alternative $A_i$ and market state $S_j$,
the company estimates the following quantities:

\begin{itemize}
	\item $q(A_i,S_j)$: number of units sold when production alternative
	$A_i$ is used and state $S_j$ occurs. This quantity depends on both the
	production system and market conditions because the production system
	affects production capacity, while the market state affects the level of
	demand.
	
	\item $r(A_i,S_j)$: revenue per unit, measured in million COP, when
	alternative $A_i$ is used and state $S_j$ occurs. This value may depend
	on both the production system and market conditions because the production
	system can affect the product offered, while market conditions can affect
	the price received per unit.
	
	\item $c(A_i,S_j)$: variable cost per unit, measured in million COP, when
	alternative $A_i$ is used and state $S_j$ occurs. This cost depends jointly
	on the production system and market conditions because the technology
	determines production requirements, while the market state affects input
	prices.
	
	\item $f_r(A_i,S_j)$: fixed revenue, measured in million COP, associated
	with alternative $A_i$ when state $S_j$ occurs, such as contractual
	payments or other revenues that do not depend directly on the number of
	units sold.
	
	\item $f_c(A_i,S_j)$: fixed cost, measured in million COP, associated
	with alternative $A_i$ when state $S_j$ occurs, such as maintenance,
	contractual, or operating costs that do not depend directly on the number
	of units sold.
\end{itemize}
...
For each combination of alternative and state, total revenue, measured in
million COP, is calculated as
...
and total cost, also measured in million COP, is
...
Therefore, the payoff, representing profit and measured in million COP, is
...
{Strong demand with favourable input-market conditions; Limited demand with expensive input-market conditions}
{Product demand and input-market conditions revealed after the production-system decision}
{Profit, measured in million COP, for each production-system--state pair}

For each alternative--state pair, total revenue is calculated as
\[
R(A_i,S_j)
=
q(A_i,S_j)r(A_i,S_j)
...
f_r(A_i,S_j),
\]
and total cost is calculated as
\[
C(A_i,S_j)
=
q(A_i,S_j)c(A_i,S_j)
...
Therefore, the payoff, measured in million COP, is
\[
P_{ay}(A_i,S_j)
=
R(A_i,S_j)-C(A_i,S_j).
...
All revenue, cost, and payoff values in the calculations below are expressed
in million COP. For every alternative--state pair, calculate
\[
\begin{aligned}
	R(A_i,S_j)
	&=
...
The resulting payoff table, with payoffs expressed in million COP, is:

\begin{center}
	\begin{tabular}{lrr}
		\toprule
		Alternative
		& \multicolumn{1}{c}{Strong demand and}
		& \multicolumn{1}{c}{Limited demand and} \\
		& \multicolumn{1}{c}{favourable input-market conditions}
		& \multicolumn{1}{c}{expensive input-market conditions} \\
		\midrule
		Conventional assembly system & 71 & 17 \\
		Automated assembly system & 87 & 24 \\
		\bottomrule
	\end{tabular}
\end{center}
